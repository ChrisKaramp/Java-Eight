...
public class Widget_getter {
    
    private String[] dataFile1D;

    public Widget_getter(ArrayList<Panel_using_array> panelsToBeRan, String yearSelected) {
        if (panelsToBeRan == null || panelsToBeRan.isEmpty()) {
            ...
            for (int currentPanelIndex = 0;
                ...
                for (Component component : panelsToBeRan.get(currentPanelIndex).getComponents()) {
                    ...
                    switch (component) {
                        case JLabel jlabel -> {
                            //System.out.println("Component no " + cioap + " (in this panel:) " + ciotp + " is a JLABEL with value: " + jlabel.getText());
                        ...
                        case Listening_spinner listening_spinner -> {
                            //System.out.println("Component no " + cioap + "(in this panel:)" + ciotp + " is a LISTENING SPINNER with value: " + listening_spinner.getValue());
                            // if panel has 2 columns --> Budget Origin frame
                            if (panelsToBeRan.get(currentPanelIndex).getPanelGridColumns() == 2) {                            
                                if (ciotp > 0) {
                                // skip first JLabel in panel (it's the title)
